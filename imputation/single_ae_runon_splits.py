...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run SimpleAE imputation across pre-generated splits.")
    ...
    for split_number in range(1, num_splits + 1):
        ...
        for src_name, tgt_name in itertools.product(maps.keys(), maps.keys()):
            ...
            model, _norms, tgt_pred_n, tgt_pred = train_cross_map_with_simpleae(
                ...
                impute_mask=tgt_train_mask, seed=seed
            )



            tgt_df = tensor_to_hgvs_dataframe(tgt_pred, mthfr_proteinseq_tgt, amino_acid_dict_num_to_3)
            ...
            merged = (
                train_df[['hgvs_pro', src_name+'_score']]
                .merge(train_mask_source_df[['hgvs_pro', 'seen_in_training_src']], on='hgvs_pro', how='left')
                .merge(tgt_df[['hgvs_pro', tgt_name+'_imputed']], on='hgvs_pro', how='left')
