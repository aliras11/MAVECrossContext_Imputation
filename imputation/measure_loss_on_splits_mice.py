...
###measure losses on MICE PMM outputs###
...
for r in rates:
    print(f"\nTest fraction r: {r}")
    mice_model_out = f"mice_test_frac_{r}" #open relevant test fraction folder for model outputs
    mice_model_out = base_dir / Path(mice_model_out) #add the base directory path so that it can be found
...
combined_results.to_csv(output_dir / "mice_loss_measurements_all_splits_rates2.csv", index=False)
