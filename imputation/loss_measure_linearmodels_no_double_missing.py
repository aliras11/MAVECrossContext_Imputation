"""Measure losses on linear model outputs from no-double-missing splits.

Usage:
    python loss_measure_linearmodels_no_double_missing.py
    python loss_measure_linearmodels_no_double_missing.py --target av12 --rates 10 40
"""
...
if __name__ == "__main__":
    ...
    for target in args.target:
        ...
        for r in args.rates:
            ...
            linear_model_out = tgt_dir / f"linear_model_output_{r}"
            ...
            for split_file in sorted(masked_data_folder.iterdir()):
                ...
                for model_out_split in sorted(linear_model_out.iterdir()):
                    ...
                    for file in sorted(model_out_split.iterdir()):
                        ...
                        rmse_test = rmse(temp_regression_loss[tgt_score + '_true'], temp_regression_loss[tgt_score])
                        rmse_train = rmse(
    ...
    if all_results:
        ...
        combined_results.to_csv(output_dir / "linear_model_loss_no_double_missing.csv", index=False)
        print(f"Saved to {output_dir / 'linear_model_loss_no_double_missing.csv'}")
    else:
