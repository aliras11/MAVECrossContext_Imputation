...
###measure losses on DualAE outputs###
...
for r in rates:
    print(f"\nTest fraction r: {r}")
    dual_ae_test_frac_folder = f"Dual_AE3_testfrac{r}" #open relevant test fraction folder for model outputs
    dual_ae_test_frac_folder = base_dir / Path(dual_ae_test_frac_folder) #add the base directory path so that it can be found
    ...
    for split_file in masked_data_folder.iterdir():
        ...
        if split_file.is_file() and split_file.name.startswith("train_split_"):
            ...
            for model_out_split in dual_ae_test_frac_folder.iterdir():
                    ...
                    for file in model_out_split.iterdir():
                        if file.is_file() and file.suffix == ".csv":
                            ...
                            model_output_df['loss_mask'] = np.select(
                                ...
                                ["double_missing", "regression_test_loss", "training", "no_src_prediction"], default="no_category"
                            )
...
combined_results.to_csv(output_dir / "dual_AE3_rmse_results.csv", index=True)
